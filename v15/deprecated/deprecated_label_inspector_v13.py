"""
DEPRECATED: Label Inspector for v13 Cache Format

⚠️  THIS FILE IS DEPRECATED ⚠️
Use the new v14 label inspector instead:
    python -m v7.cache_v14.label_inspector

This file is kept for reference only and works with old v13 cache format.
For v14 caches, use the new inspector with clean architecture and better features.
...
TF_VIEW_SETS = {
    'mixed': ['5min', '15min', '1h', 'daily'],      # Default: mixed timeframes
    'intraday': ['5min', '1h', '2h', '4h'],         # Short-term trading
    'multiday': ['daily', 'weekly', 'monthly', '3month'], # Longer-term trends
}
TF_VIEW_NAMES = ['mixed', 'intraday', 'multiday']
DISPLAY_TIMEFRAMES = TF_VIEW_SETS['mixed']  # Default view
...
def plot_tf_panel(
    ...
    """
    # Handle missing data or unavailable window
    if df_tf is None:
        # No data at all for this TF
    ...
    if channel is None:
        # Window not available for this TF - show grayed out N/A
        ...
        # Muted title
        if display_window is not None:
            ax.set_title(f"{tf_name} - Window {display_window} N/A",
                         fontsize=11, fontweight='bold', color='gray')
        else:
            ax.set_title(f"{tf_name} (No Valid Channel)",
                         fontsize=11, fontweight='bold', color='gray')

        # Faded grid for visual consistency
...
class SampleBrowser:
    ...
    def __init__(self, samples: list, tsla_df: pd.DataFrame, window: int = 50, min_cycles: int = 1):
        ...
        # Timeframe view cycling state
        self.tf_view_idx = 0  # Index into TF_VIEW_NAMES (0 = mixed, 1 = intraday, 2 = multiday)
    ...
    def _print_controls(self):
        ...
        print("  t          : Swap timeframe views (mixed -> intraday -> multiday -> mixed)")
        print("  q/ESC      : Quit")
        print("="*50 + "\n")
    ...
    def _create_figure(self):
        ...
        ax_goto = plt.axes([0.3, 0.01, 0.12, 0.04])
        ax_flagged = plt.axes([0.45, 0.01, 0.12, 0.04])
        ax_swap_tf = plt.axes([0.6, 0.01, 0.15, 0.04])

        self.btn_prev = Button(ax_prev, '← Previous')
        self.btn_next = Button(ax_next, 'Next →')
        self.btn_goto = Button(ax_goto, 'Jump to...')
        self.btn_flagged = Button(ax_flagged, '⚠ Flagged')
        self.btn_swap_tf = Button(ax_swap_tf, '🔄 Swap TFs')
    ...
    def _cycle_timeframe_view(self):
        """Cycle through timeframe views: mixed -> intraday -> multiday -> mixed."""
        self.tf_view_idx = (self.tf_view_idx + 1) % len(TF_VIEW_NAMES)
        view_name = TF_VIEW_NAMES[self.tf_view_idx]
        timeframes = TF_VIEW_SETS[view_name]
    ...
    def _update_plot(self):
        ...
        # Clear previous subplots (keep buttons - first 5 axes)
        for ax in self.fig.axes[5:]:  # Skip button axes (now 5 buttons)
            ax.remove()
