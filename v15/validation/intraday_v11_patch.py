#!/usr/bin/env python3
"""V11 patch: Run experiments 9-12 that crashed due to Unicode arrow char.
Reuses all the same signal functions from V11."""
import os, sys, time
import numpy as np
import pandas as pd
...
def sig_stacked_slope_agree(i, f5m, ctx, params):
    ...
    hs = ctx['1h_slope'][i] if '1h_slope' in ctx else np.nan
    h4s = ctx['4h_slope'][i] if '4h_slope' in ctx else np.nan
    min_slopes_up = params.get('min_slopes_up', 2)
    count = 0
...
def sig_stacked_rsi_div(i, f5m, ctx, params):
    ...
    min_rsi_slope = params.get('min_rsi_slope', 0.5)
...
def sig_stacked_gap(i, f5m, ctx, params):
    ...
    if base is None: return None
    gap = f5m['gap_pct'].iloc[i] if 'gap_pct' in f5m.columns else np.nan
    max_gap = params.get('max_gap', 0.0)
...
def main():
    print("="*70); print("V11 PATCH: Experiments 9-12"); print("="*70); _flush()
    df1m = load_1min()
    ...
    # BONUS: Enhanced conf + wider + PM + aggressive sizing
    print("\n--- Enhanced conf + wider + PM + $100K-$500K ---"); _flush()
    trades = simulate(f5m, sig_stacked_enhanced_conf, "enh_pm_agg", pw, precomp,
                     conf_size=True, base_capital=100_000.0, max_capital=500_000.0,
                     tod_start=dt.time(13,0), tod_end=dt.time(15,25))
    report(trades, "Enhanced + wider + PM + $100K-$500K")

    print("\n" + "="*70)
    print("V11 PATCH DONE.")
    print("="*70); _flush()
