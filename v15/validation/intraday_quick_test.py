#!/usr/bin/env python3
"""Quick focused test: VWAP no-turn, best configs, cooldown sweep.
Flushes output after every print."""
import os, sys, time
import numpy as np
import pandas as pd
...
def channel_position(close_arr, window=60):
    ...
    cy = np.cumsum(close)
    sy = np.full(n,np.nan); sy[w-1]=cy[w-1]
...
def simulate(f5m, signal_fn, name, params, precomp,
    ...
    for i in range(n):
        ...
        if in_trade:
            ...
            if xp is not None:
                xa=xp*(1-SLIPPAGE_PCT); sh=max(1,int(CAPITAL*conf/ep))
...
# ================================================================
def main():
    print("="*70); print("QUICK TEST: KEY EXPERIMENTS"); print("="*70); _flush()
    df1m=load_1min()
    ...
    # ── TEST 1: Baseline signals ──
    print("\n"+"="*70); print("TEST 1: Baselines (with turn)"); print("="*70); _flush()
    for nm,fn in [('VWAP',sig_vwap),('Div',sig_div),('Union',sig_union)]:
        trades=simulate(f5m,fn,nm,dp,precomp)
        report(trades,nm)

    # ── TEST 2: No turn requirement ──
    print("\n"+"="*70); print("TEST 2: No turn requirement"); print("="*70); _flush()
    p2=dict(dp,need_turn=False)
    for nm,fn in [('VWAP_noTurn',sig_vwap),('Div_noTurn',sig_div),('Union_noTurn',sig_union)]:
        trades=simulate(f5m,fn,nm,p2,precomp)
        report(trades,nm)

    # ── TEST 3: Cooldown + trades/day sweep ──
    print("\n"+"="*70); print("TEST 3: Cooldown + trades/day (Union with turn)"); print("="*70); _flush()
    for cd in [0,2,3,6,12]:
        for mtd in [1,2,3,5]:
            trades=simulate(f5m,sig_union,'U',dp,precomp,cd=cd,mtd=mtd)
            if trades:
                n=len(trades); pnls=[t[6] for t in trades]
                w=sum(1 for p in pnls if p>0); wr=w/n*100; total=sum(pnls); bl=min(pnls)
                oos=[t for t in trades if t[0]>TEST_END]; on=len(oos)
                ow=sum(1 for t in oos if t[6]>0); op=sum(t[6] for t in oos)
                print(f"  cd={cd:2d} mtd={mtd}: {n:4d}t {wr:5.1f}% ${total:+10,.0f} BL=${bl:+,.0f} OOS:{on}t {ow}W ${op:+,.0f}")
                _flush()

    # ── TEST 4: Trail sweep ──
    print("\n"+"="*70); print("TEST 4: Trail sweep (Union with turn)"); print("="*70); _flush()
    for tb in [0.005,0.008,0.010,0.015,0.020,0.030]:
        for tpow in [2,4,6,8,12]:
            trades=simulate(f5m,sig_union,'U',dp,precomp,tb=tb,tp=tpow)
            if trades and len(trades)>=5:
                n=len(trades); pnls=[t[6] for t in trades]
                w=sum(1 for p in pnls if p>0); wr=w/n*100; total=sum(pnls); bl=min(pnls)
                f=" ***" if wr>=90 else ""
                print(f"  tb={tb:.3f} tp={tpow:2d}: {n:4d}t {wr:5.1f}% ${total:+10,.0f} BL=${bl:+,.0f}{f}")
                _flush()

    # ── TEST 5: VWAP parameter sweep (higher WR focus) ──
    print("\n"+"="*70); print("TEST 5: VWAP param sweep (WR focus)"); print("="*70); _flush()
            ...
            for dm in [0.10,0.15,0.20,0.30]:
                for hm in [0.10,0.15,0.20]:
                    for nt in [True,False]:
                        p=dict(dp,vwap_thresh=vt,f5_thresh=f5t,d_min=dm,h1_min=hm,need_turn=nt)
                        trades=simulate(f5m,sig_vwap,'V',p,precomp)
                        if trades and len(trades)>=10:
                            n=len(trades); pnls=[t[6] for t in trades]
                            w=sum(1 for p in pnls if p>0); wr=w/n*100; total=sum(pnls); bl=min(pnls)
                            nts="T" if nt else "N"
                            if wr>=82 or (wr>=78 and total>350000):
                                print(f"  vwap<{vt:.2f} 5m<{f5t:.2f} d>{dm:.2f} h1>{hm:.2f} {nts}: "
                                      f"{n:4d}t {wr:5.1f}% ${total:+10,.0f} BL=${bl:+,.0f}")
                                _flush()

    # ── TEST 6: Div parameter sweep ──
    print("\n"+"="*70); print("TEST 6: Div param sweep"); print("="*70); _flush()
    for dv in [0.20,0.25,0.30,0.35,0.40,0.50]:
        for f5t in [0.20,0.25,0.30,0.35]:
            for nt in [True,False]:
                p=dict(dp,div_thresh=dv,f5_thresh=f5t,need_turn=nt)
                trades=simulate(f5m,sig_div,'D',p,precomp)
                if trades and len(trades)>=10:
                    n=len(trades); pnls=[t[6] for t in trades]
                    w=sum(1 for p in pnls if p>0); wr=w/n*100; total=sum(pnls); bl=min(pnls)
                    nts="T" if nt else "N"
                    if wr>=65 or total>400000:
                        print(f"  div>{dv:.2f} 5m<{f5t:.2f} {nts}: "
                              f"{n:4d}t {wr:5.1f}% ${total:+10,.0f} BL=${bl:+,.0f}")
                        _flush()

    # ── TEST 7: Best Union configs ──
    print("\n"+"="*70); print("TEST 7: Best union combos"); print("="*70); _flush()
    for vt in [-0.20,-0.30,-0.50]:
        for dv in [0.30,0.35,0.40]:
            for f5t in [0.25,0.30,0.35]:
                for nt in [True,False]:
                    for cd in [0,3,6]:
                        for mtd in [2,3]:
                            p=dict(dp,vwap_thresh=vt,div_thresh=dv,f5_thresh=f5t,need_turn=nt)
                            trades=simulate(f5m,sig_union,'U',p,precomp,cd=cd,mtd=mtd)
                            if trades and len(trades)>=10:
                                n=len(trades); pnls=[t[6] for t in trades]
                                ...
                                op=sum(t[6] for t in oos)
                                nts="T" if nt else "N"
                                if total>500000 or (wr>=70 and total>400000):
                                    print(f"  vt={vt:.1f} dv={dv:.1f} f5={f5t:.1f} {nts} cd={cd} mtd={mtd}: "
                                          f"{n:4d}t {wr:5.1f}% ${total:+10,.0f} BL=${bl:+,.0f} "
                                          f"OOS:{on}t {ow}W ${op:+,.0f}")
                                    _flush()
