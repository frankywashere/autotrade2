"""
Evaluator for OpenEvolve Phase C: Surfer-ML exit logic evolution.
...
The candidate program defines get_effective_stop() and check_exits() functions.
These are monkey-patched onto SurferMLAlgo BEFORE the backtest runs.
The ML entry signal is FROZEN — only exit logic changes.
...
def evaluate(program_path: str) -> dict:
    """Evaluate an exit logic candidate. Returns score dict for OpenEvolve."""
    try:
        data = _load_data()
    except Exception as e:
        return {'combined_score': 0.0, 'error': f'data load failed: {e}'}

    # Import the candidate's exit functions
    try:
        spec = importlib.util.spec_from_file_location('candidate', program_path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)

        # Candidate MUST define these two functions
        candidate_get_effective_stop = getattr(mod, 'get_effective_stop', None)
        candidate_check_exits = getattr(mod, 'check_exits', None)

        if candidate_get_effective_stop is None:
            return {'combined_score': 0.0, 'error': 'missing get_effective_stop()'}
        if candidate_check_exits is None:
            return {'combined_score': 0.0, 'error': 'missing check_exits()'}

    except Exception as e:
        return {'combined_score': 0.0, 'error': f'import failed: {e}'}

    # Run backtester with monkey-patched exit logic
    try:
        ...
        config.stop_update_secs = 60
        config.stop_check_secs = 5
        config.grace_ratchet_secs = 60
        ...
        # ── Monkey-patch exit logic ──────────────────────────────────
        # Bind the candidate functions as methods on this algo instance
        algo.get_effective_stop = types.MethodType(candidate_get_effective_stop, algo)
        algo.check_exits = types.MethodType(candidate_check_exits, algo)
