#!/usr/bin/env python3
"""V10: Afternoon-only optimization.
V9 found: afternoon (13:00-15:25) = 89.9% WR vs full day 86.9%.
Power hour (14:30-15:25) = 91.3%.
Test: wider params + afternoon window + conf sizing + vol filters.
Also: what about MidDay with volume filter? Can we rescue 11-13?
"""
import os, sys, time
import numpy as np
import pandas as pd
...
def main():
    print("="*70); print("V10: AFTERNOON OPTIMIZATION"); print("="*70); _flush()
    df1m = load_1min()
    ...
    # ── EXP 1: Afternoon stacked with wider params + 75K-300K ──
    print("\n"+"="*70)
    print("EXP 1: Afternoon (13:00-15:25) stacked + wider params + 75K-300K")
    print("="*70); _flush()
    for vt in [-0.10, -0.20, -0.30, -0.50]:
        for dvt in [0.20, 0.25, 0.30, 0.35]:
            for f5t in [0.25, 0.30, 0.35]:
                p = dict(dp, vwap_thresh=vt, div_thresh=dvt, f5_thresh=f5t)
                label = f"PM vw<{vt:.1f} dv>{dvt:.2f} f5<{f5t:.2f}"
                ...
                if trades and len(trades) >= 30:
                    ...
                    if total > 2500000 or (wr >= 90 and total > 1500000):
                        report_short(trades, label)

    # ── EXP 2: Afternoon VWAP-only with wider params ──
    print("\n"+"="*70)
    print("EXP 2: Afternoon VWAP-only wider params + 75K-300K")
    print("="*70); _flush()
    for vt in [-0.10, -0.20, -0.30, -0.50]:
        for f5t in [0.25, 0.30, 0.35, 0.40]:
            p = dict(dp, vwap_thresh=vt, f5_thresh=f5t)
            label = f"PM VW vw<{vt:.1f} f5<{f5t:.2f}"
            trades = simulate(f5m, sig_vwap, label, p, precomp, conf_size=True,
                            tod_start=dt.time(13,0), tod_end=dt.time(15,25))
            report_short(trades, label)

    # ── EXP 3: Union afternoon + wider ──
    print("\n"+"="*70)
    print("EXP 3: Afternoon Union wider params + 75K-300K")
    print("="*70); _flush()
    for vt in [-0.10, -0.20, -0.30]:
        for dvt in [0.20, 0.25, 0.30, 0.35]:
            p = dict(dp, vwap_thresh=vt, div_thresh=dvt, f5_thresh=0.30)
            label = f"PM Un vw<{vt:.1f} dv>{dvt:.2f}"
            trades = simulate(f5m, sig_union, label, p, precomp, conf_size=True,
                            tod_start=dt.time(13,0), tod_end=dt.time(15,25))
            report_short(trades, label)

    # ── EXP 4: Full reports on best afternoon configs ──
    print("\n"+"="*70)
    print("EXP 4: Full year-by-year - BEST AFTERNOON CONFIGS")
    print("="*70); _flush()

    # A: PM Stacked wider vw<-0.2 dv>0.20 f5<0.30
    print("\n--- PM Stacked vw<-0.2 dv>0.20 f5<0.30 ---"); _flush()
    p = dict(dp, vwap_thresh=-0.20, div_thresh=0.20, f5_thresh=0.30)
    trades = simulate(f5m, sig_stacked, "PMa", p, precomp, conf_size=True,
                     tod_start=dt.time(13,0), tod_end=dt.time(15,25))
    report(trades, "PM Stack vw<-0.2 dv>0.20")

    # B: PM Stacked tight vw<-0.5 dv>0.35
    print("\n--- PM Stacked vw<-0.5 dv>0.35 ---"); _flush()
    p = dict(dp, vwap_thresh=-0.50, div_thresh=0.35)
    trades = simulate(f5m, sig_stacked, "PMb", p, precomp, conf_size=True,
                     tod_start=dt.time(13,0), tod_end=dt.time(15,25))
    report(trades, "PM Stack vw<-0.5 dv>0.35")

    # C: PM VWAP wider vw<-0.10 f5<0.35
    print("\n--- PM VWAP vw<-0.10 f5<0.35 ---"); _flush()
    p = dict(dp, vwap_thresh=-0.10, f5_thresh=0.35)
    trades = simulate(f5m, sig_vwap, "PMc", p, precomp, conf_size=True,
                     tod_start=dt.time(13,0), tod_end=dt.time(15,25))
    report(trades, "PM VWAP vw<-0.10 f5<0.35")

    # D: Full-day champion for comparison
    print("\n--- Full-day champion (baseline) ---"); _flush()
    p = dict(dp)
    trades = simulate(f5m, sig_stacked, "FD", p, precomp, conf_size=True)
    report(trades, "Full-day Stack N vr>0.8 75K-300K")

    # E: PM + MidDay with vol>1.5 (rescue midday with strict vol filter)
    print("\n--- MidDay 11:00-13:00 with vol>1.5 ---"); _flush()
    p = dict(dp, min_vol_ratio=1.5)
    trades = simulate(f5m, sig_stacked, "MDv", p, precomp, conf_size=True,
                     tod_start=dt.time(11,0), tod_end=dt.time(13,0))
    report(trades, "MidDay Stack vr>1.5")
