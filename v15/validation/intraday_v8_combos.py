#!/usr/bin/env python3
"""V8: Combine V7's best discoveries.
Key findings to combine:
1. Volume ratio filter: 84-87.6% WR boost
2. noTurn: +21% trades with ~2% WR loss
3. High mtd (8-10): linear PnL scaling
4. Conf sizing: 1.76x multiplier
5. Stacked signals: 82-85% WR
6. RSI5 filter: marginal WR boost

Tests: combined filters, optimal config search, full year reports.
"""
import os, sys, time
import numpy as np
import pandas as pd
...
def simulate(f5m, signal_fn, name, params, precomp,
             tb=0.010, tp=4, cd=0, mtd=5, conf_size=False,
...
def report_short(trades, label):
    ...
    print(f"  {label}: {n:5d}t {wr:5.1f}% ${total:+10,.0f} BL=${bl:+,.0f} "
          f"TR=${tp_:+,.0f} TS=${tsp:+,.0f} OOS:{on}t {ow}W ${op:+,.0f}")
    _flush()
...
def sig_vwap(i, f5m, ctx, params):
    ...
    # RSI5 filter
    max_rsi5 = params.get('max_rsi5', 100)
    if max_rsi5 < 100:
        rsi5 = f5m['rsi5'].iloc[i] if 'rsi5' in f5m.columns else np.nan
        if not np.isnan(rsi5) and rsi5 > max_rsi5: return None
...
# ================================================================
def main():
    print("="*70); print("V8: COMBINED FILTER OPTIMIZATION"); print("="*70); _flush()
    df1m = load_1min()
    ...
    # ── EXP 1: VWAP + Vol filter + noTurn + mtd sweep ──
    print("\n"+"="*70)
    print("EXP 1: VWAP + Vol filter combos (cd=0)")
    print("="*70); _flush()
    for vr in [0.0, 0.8, 1.0, 1.2]:
        for nt in [True, False]:
            for mtd in [5, 8, 10]:
                p = dict(dp, need_turn=nt, min_vol_ratio=vr)
                label = f"VW vr>{vr:.1f} {'T' if nt else 'N'} m={mtd}"
                trades = simulate(f5m, sig_vwap, label, p, precomp, cd=0, mtd=mtd)
                report_short(trades, label)

    # ── EXP 2: VWAP + Vol + RSI5 double filter ──
    print("\n"+"="*70)
    print("EXP 2: VWAP + Vol + RSI5 double filter (cd=0, mtd=5)")
    print("="*70); _flush()
    for vr in [0.8, 1.0, 1.2]:
        for rsi in [30, 40, 50]:
            for nt in [True, False]:
                p = dict(dp, need_turn=nt, min_vol_ratio=vr, max_rsi5=rsi)
                label = f"VW vr>{vr:.1f} rsi<{rsi} {'T' if nt else 'N'}"
                trades = simulate(f5m, sig_vwap, label, p, precomp, cd=0, mtd=5)
                report_short(trades, label)

    # ── EXP 3: Stacked + Vol filter ──
    print("\n"+"="*70)
    print("EXP 3: Stacked (VWAP+Div) + Vol filter (cd=0)")
    print("="*70); _flush()
    for vr in [0.0, 0.8, 1.0, 1.2]:
        for nt in [True, False]:
            for mtd in [5, 8, 10]:
                p = dict(dp, need_turn=nt, min_vol_ratio=vr)
                label = f"Stack vr>{vr:.1f} {'T' if nt else 'N'} m={mtd}"
                trades = simulate(f5m, sig_stacked, label, p, precomp, cd=0, mtd=mtd)
                report_short(trades, label)

    # ── EXP 4: Union + Vol filter + mtd sweep ──
    print("\n"+"="*70)
    print("EXP 4: Union + Vol filter (cd=0)")
    print("="*70); _flush()
    for vr in [0.0, 0.8, 1.0, 1.2]:
        for nt in [True, False]:
            for mtd in [5, 8]:
                p = dict(dp, need_turn=nt, min_vol_ratio=vr)
                label = f"Union vr>{vr:.1f} {'T' if nt else 'N'} m={mtd}"
                trades = simulate(f5m, sig_union, label, p, precomp, cd=0, mtd=mtd)
                report_short(trades, label)

    # ── EXP 5: Conf sizing on best combos ──
    print("\n"+"="*70)
    print("EXP 5: Confidence sizing on top configs")
    print("="*70); _flush()
    configs = [
        ("VWAP N vr>1.0 m=8", sig_vwap, dict(dp, need_turn=False, min_vol_ratio=1.0), 8),
        ("VWAP N vr>0.8 m=10", sig_vwap, dict(dp, need_turn=False, min_vol_ratio=0.8), 10),
        ("VWAP T vr>1.0 m=8", sig_vwap, dict(dp, need_turn=True, min_vol_ratio=1.0), 8),
        ("VWAP N m=10", sig_vwap, dict(dp, need_turn=False), 10),
        ("Stack N vr>0.8 m=8", sig_stacked, dict(dp, need_turn=False, min_vol_ratio=0.8), 8),
        ("Stack N m=10", sig_stacked, dict(dp, need_turn=False), 10),
        ("Union T m=5", sig_union, dict(dp, need_turn=True), 5),
    ]
    for label, fn, p, mtd in configs:
        t_fix = simulate(f5m, fn, label+" fix", p, precomp, cd=0, mtd=mtd, conf_size=False)
        t_scl = simulate(f5m, fn, label+" scl", p, precomp, cd=0, mtd=mtd, conf_size=True)
        report_short(t_fix, label + " $100K")
        report_short(t_scl, label + " SCALED")

    # ── EXP 6: FULL REPORTS on top 5 configs ──
    print("\n"+"="*70)
    print("EXP 6: Full year-by-year reports - TOP CONFIGS")
    print("="*70); _flush()

    # Config 1: VWAP N vr>1.0 mtd=10 conf-sized (predicted best)
    print("\n--- Config 1: VWAP noTurn vol>1.0 mtd=10 CONF-SIZED ---"); _flush()
    p = dict(dp, need_turn=False, min_vol_ratio=1.0)
    trades = simulate(f5m, sig_vwap, "C1", p, precomp, cd=0, mtd=10, conf_size=True)
    report(trades, "VWAP N vr>1.0 m=10 SCALED")

    # Config 2: VWAP N vr>0.8 mtd=10 conf-sized
    print("\n--- Config 2: VWAP noTurn vol>0.8 mtd=10 CONF-SIZED ---"); _flush()
    p = dict(dp, need_turn=False, min_vol_ratio=0.8)
    trades = simulate(f5m, sig_vwap, "C2", p, precomp, cd=0, mtd=10, conf_size=True)
    report(trades, "VWAP N vr>0.8 m=10 SCALED")

    # Config 3: Stacked N vr>0.8 mtd=10 conf-sized
    print("\n--- Config 3: Stacked noTurn vol>0.8 mtd=10 CONF-SIZED ---"); _flush()
    p = dict(dp, need_turn=False, min_vol_ratio=0.8)
    trades = simulate(f5m, sig_stacked, "C3", p, precomp, cd=0, mtd=10, conf_size=True)
    report(trades, "Stacked N vr>0.8 m=10 SCALED")

    # Config 4: VWAP T vr>1.2 mtd=8 (high WR focus)
    print("\n--- Config 4: VWAP turn vol>1.2 mtd=8 (WR focus) ---"); _flush()
    p = dict(dp, need_turn=True, min_vol_ratio=1.2)
    trades = simulate(f5m, sig_vwap, "C4", p, precomp, cd=0, mtd=8)
    report(trades, "VWAP T vr>1.2 m=8")

    # Config 5: VWAP N mtd=10 no vol filter conf-sized (raw max)
    print("\n--- Config 5: VWAP noTurn mtd=10 no-vol CONF-SIZED ---"); _flush()
    p = dict(dp, need_turn=False)
    trades = simulate(f5m, sig_vwap, "C5", p, precomp, cd=0, mtd=10, conf_size=True)
    report(trades, "VWAP N m=10 SCALED")
