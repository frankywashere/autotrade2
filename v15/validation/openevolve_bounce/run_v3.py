"""
Launcher for OpenEvolve bounce signal v3 (TSLA RSI + 52wk SMA features).
Uses port 5561 to avoid conflict with v2 run on 5560.
"""
...
def main():
    parser = argparse.ArgumentParser(description='OpenEvolve Bounce Signal v3')
    parser.add_argument('--port', type=int, default=5561)
    parser.add_argument('--iterations', type=int, default=300)
    args = parser.parse_args()

    print(f"\n{'='*60}")
    print(f"OpenEvolve Bounce Signal v3 (TSLA RSI + 52wk SMA)")
    ...
    config_path = os.path.join(base_dir, 'config_v3.yaml')
    initial_program = os.path.join(base_dir, 'initial_program_v3.py')
    evaluator_path = os.path.join(base_dir, 'evaluator_v3.py')
