// Temporary file containing the Pass 3 implementation
// This will be inserted into scanner.cpp
...
std::vector<ChannelSample> Scanner::process_channel_batch(
    ...
    for (const auto& work_item : batch) {
        ...
        try {
            ...
            int idx_5min = primary_channel.end_idx;
            ...
            if (config_.verbose) {
                std::cerr << "ERROR processing channel (" << primary_tf << ","
